...
class Goods extends Controller
{
    ...
    // 抢购中列表
    public function ingLists($category_id = 0)
    {

        $model = new GoodsModel;
        $list = $model->getList(10, $category_id,'','all',false,15,'ing');
        return $this->renderSuccess(compact('list'));
    }
    // 秒杀预告列表
    public function beforeLists($category_id = 0)
    {
        $model = new GoodsModel;
        $list = $model->getList(10, $category_id,'','all',false,15,'before');
        return $this->renderSuccess(compact('list'));
    }
