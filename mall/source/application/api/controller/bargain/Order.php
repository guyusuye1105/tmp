...
class Order extends Controller
{
    ...
    )
    {
        ...
        // 创建订单
        if ($order_id = $model->createOrder($this->user['user_id'], $order, $active_id, $coupon_id, $remark)) {
            // 生成active表信息
            $goodsModel = new GoodsModel;
            $param['user_id'] = $this->user['user_id'];
            $param['goods'] = $goodsModel->where('goods_id='.$goods_id)->find()->toArray();
            $param['active_id'] = 0;
            $param['order_id'] = $order_id;
            $model->saveBargainActive($param);

            return $this->renderSuccess( '砍价订单创建成功');
        }
        return $this->renderError($model->getError() ?: '订单创建失败');
    }

    // 帮砍价
    public function helpBargain($active_id,$creator_id)
    {
        $model = new OrderModel;
       /* $activeModel = new ActiveModel;
        ...
        $res2 = $model->saveBargainActive($param);
        if($res2){
            return $this->renderSuccess( '帮砍价成功');
        }else{
            return $this->renderError( $model->getError() ?: '帮砍价失败');
        }
    }
    /**
     * 构建微信支付
    ...
    private function unifiedorder($order, $user)
    {
        ...
        $payment = $WxPay->unifiedorder($order['order_no'], $user['open_id'], $order['pay_price'], 'bargain');
        // 记录prepay_id
    ...
    public function pay($order_id)
    {
        ...
        $wxConfig = WxappModel::getWxappCache();
        $WxPay = new WxPay($wxConfig);
        $payment = $WxPay->unifiedorder($order['order_no'], $this->user['open_id'], $order['pay_price'], 'bargain');
        // 记录prepay_id
        $model = new WxappPrepayIdModel;
