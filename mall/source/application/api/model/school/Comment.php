...
use app\common\model\school\Comment as CommentModel;
...
class Comment extends CommentModel
{
     ...
     */
    public function getGoodsCommentList($goods_id,$order_id)
    {
        // 筛选条件
        $filter = [
            'goods_id' => $goods_id,
     ...
     * @throws \Exception
     */
    public function addForOrder($order, $goodsList, $formJsonData,$teacher_id)
    {
        // 生成 formData
        $formData = $this->formatFormData($formJsonData);
        // 生成评价数据
        $data = $this->createCommentData($order['user_id'], $order['order_id'], $goodsList, $formData,$teacher_id);
        if (empty($data)) {
            $this->error = '没有输入评价内容';
            return false;
        }
     ...
     */
    private function createCommentData($user_id, $order_id, &$goodsList, &$formData,$teacher_id)
    {
        $data = [];
        foreach ($goodsList as $goods) {
            //dump($goods['order_goods_id']);die;
            if (!isset($formData[$goods['order_goods_id']])) {
                throw new BaseException(['msg' => '提交的数据不合法1']);
            }
            ...
            !empty($item['content']) && $data[$goods['order_goods_id']] = [
                ...
                'subject_star' => $item['subject_star'],
                'teacher_star' => $item['teacher_star'],
