...
class Active extends ActiveModel
{
    ...
    public function onCreate($creator_id, $order_id, $goods)
    {
        ...
        $param = array(
            ...
            'orders_id' =>$order_id,
            'flashsale_x'=>$goods['flashsale_x'],
            'flashsale_y'=>$goods['flashsale_y'],
        );
    ...
    public function onUpdate($creator_id, $user_id,$order)
    {
        ...
        $kan_money = rand($active['flashsale_x']*100,$active['flashsale_y']*100)/100;
        ...
        if($order['min_price']>$param2['pay_price']){
            ...
            if($kan_money == 0){
                return array('code'=>3,'msg'=>'已经砍到了最低价，无法再秒杀');
            }
        }
            //减少订单价格
        ...
        if ($status == 20) {
            ...
            // (new Message)->flashsaleActive($model, '秒杀成功');
        }
        return array('code'=>1);
    }
