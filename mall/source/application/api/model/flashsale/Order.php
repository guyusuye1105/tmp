...
use app\common\model\flashsale\Order as OrderModel;
use app\api\model\dealer\Order as DealerOrderModel;
use app\api\model\flashsale\Setting as flashsaleSettingModel;
...
class Order extends OrderModel
{
    ...
    public function getBuyNow($user, $order_type, $goods_id, $goods_num, $goods_sku_id)
    {
        ...
        // order_type：下单类型 10=>单独购买 20=>秒杀
        $goods['goods_price'] = $order_type == 10 ? $goods['goods_sku']['goods_price']
            : $goods['goods_sku']['flashsale_price'];
        // 商品总价
        ...
        // 可用优惠券列表
        $couponList = [];
        if (flashsaleSettingModel::getItem('basic')['is_coupon']) {
    ...
    public function createOrder($user_id, $order, $active_id = null, $coupon_id = null, $remark = '')
    {
        ...
        try {
            ...
            // 记录分销商订单
            if (flashsaleSettingModel::getItem('basic')['is_dealer']) {
     ...
     * @throws \think\exception\DbException
     */
    public function saveflashsaleActive_old($goods)
    {
        // 新增/更新拼单记录
    ...
    // 更新秒杀记录
    public function saveflashsaleActive($param)
    {
        // 参与他人的拼单, 更新拼单记录
        if ($param['active_id'] > 0) {
            $ActiveModel = Active::detail($param['active_id']);
            $res = $ActiveModel->onUpdate($param['creator_id'],$param['user_id'], $param['order']);
            if($res['code'] == 1){
                return true;
            }else{
                $this->error = $res['msg'];
        ...
        // 自己发起的拼单, 新增拼单记录
        $ActiveModel = new Active;
        $ActiveModel->onCreate($param['user_id'], $param['order_id'], $param['goods']);
        return true;
    }
    ...
    private function add($user_id, &$order, $remark = '')
    {
        return $this->save([
            ...
            'min_price'=>$order['goods_list'][0]['goods_sku']['goods_price'],
        ]);
    }
...
//                'order_status' => ['<>', 20]
    ...
    {
        $res = $this->with(['goods' => ['image', 'refund'], 'address', 'express'])
            ->alias('order')
            ->field('order.*, active.status as active_status')
            ->join('flashsale_active active', 'order.active_id = active.active_id', 'LEFT')
            ->where([
