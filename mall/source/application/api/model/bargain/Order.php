...
use app\api\model\bargain\Setting as bargainSettingModel;
...
class Order extends OrderModel
{
    ...
    public function getBuyNow($user, $order_type, $goods_id, $goods_num, $goods_sku_id)
    {
        ...
        // 可用优惠券列表
        $couponList = [];
        if (bargainSettingModel::getItem('basic')['is_coupon']) {
    ...
    public function createOrder($user_id, $order, $active_id = null, $coupon_id = null, $remark = '')
    {
        ...
        try {
            ...
            // 记录分销商订单
            if (bargainSettingModel::getItem('basic')['is_dealer']) {
     ...
     * @throws \think\exception\DbException
     */
    public function saveBargainActive_old($goods)
    {
        // 新增/更新拼单记录
    ...
    // 更新砍价记录
    public function saveBargainActive($param)
    {
        // 参与他人的拼单, 更新拼单记录
    ...
    public function getList($user_id, $type = 'all')
    {
        ...
        return $this->with(['goods.image'])
            ...
            ->join('bargain_active active', 'order.active_id = active.active_id', 'LEFT')
