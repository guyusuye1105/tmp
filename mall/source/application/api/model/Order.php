...
class Order extends OrderModel
{
    ...
    public function createOrder($user_id, $order, $coupon_id = null, $remark = '')
    {
        ...
        if (empty($order['address'])) {
            $good=$order['goods_list'][0];
            if(isset($good['is_virtual']) && $good['is_virtual']==1){
                $this->error = '请先填写信息';
                return false;
            }else{
                $this->error = '请先选择收货地址';
    ...
    private function saveOrderGoods($user_id, &$order)
    {
        ...
        foreach ($order['goods_list'] as $goods) {
            ...
            $goodsList[] = [
                ...
                'subject_id' => $goods['subject_id'],
                'is_appoint_time' => $goods['is_appoint_time'],
            ];
        }
...
//            'order_status' => ['<>', 20]
    ...
    public function getAppointList($user_id)
    {
        $filter['pay_status'] = 20;
        $filter['delivery_status'] = 20;
        $filter['receipt_status'] = 10;
        $filter['order_status'] = 10;
        $filter['sn_code'] = ['<>',''];//二维串码不为空
        //$filter['goods.subject_id'] = ['<>',0];//二维串码不为空
        ...
        foreach ($res as $key=>$val){
            $goods_list[0] = $val['goods'][0];
            break;
        }
        $url = 'http://localhost/mp/addons/mall/web/index.php?s=/store/crontab/order_goods&order_goods_id='.$goods_list[0]['order_goods_id'];
        $ch  = curl_init();
        ...
        // lichenjie 如果是需要预约的商品,那么插入预约表
       /* if(isset($goods_list[0]['is_appoint_time']) && $goods_list[0]['is_appoint_time']==1){
            $param = array(
                'wxapp_id'=>self::$wxapp_id,
                'user_id'=>$goods_list[0]['user_id'],
                'subject_id'=>$goods_list[0]['subject_id'],
                'state'=>'nouse',
                'goods_id'=>$goods_list[0]['goods_id'],
                'order_id'=>$goods_list[0]['order_id'],
            );
            $result = Db('school_appoint')->insert($param);
        }
*/
