...
class User extends UserModel
{
     ...
     * @return mixed
     */
    public function getphone($post,$token){
        // 获取当前小程序信息
        $wxConfig = Wxapp::getWxappCache();

        $session_key=Cache::get($token)['session_key'];
        ...
        if (!$phone = $WxUser->getphone($encryptedData,$iv,$session_key)){
            throw new BaseException(['msg' => 'phone 获取失败']);
        }else{
            //unset($phone['watermark']);
            //拿到手机号后 更新下本地信息
            $user = self::detail(['open_id' => $open_id]);
            if($user){
                $user->save(['mobile'=>$phone['phoneNumber']]);
            }
