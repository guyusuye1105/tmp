...
class Goods extends Base
{
    ...
    public function getImage()
    {
        ...
        $backs = &$settingModel::getItem('poster')['backdrop']['src'];
        if(!empty($backs)){
            //$backdrop = $backs;
            $a = str_replace('source','web',ROOT_PATH);
            $b = strstr($backs,'assets');
            $backdrop = $a.$b;
        }else{
        ...
        $backs = &$settingModel::getItem('poster')['backdrop']['src'];
        if(!empty($backs)){
            $backdrop = $this->saveTempImage($wxappId, $backs, 'backdrop');
        }else{
            $backdrop = __DIR__ . '/resource/goods_bg.png';
        }
        $backdrop = __DIR__ . '/resource/goods_bg.png'; //强制把背景图换成空白
        // 下载商品首图
        ...
        // 拼接海报图
        $res = $this->savePoster_school($backdrop, $goodsUrl, $qrcode);
     ...
     * @throws \Exception
     */
    private function savePoster_school($backdrop, $goodsUrl, $qrcode)
    {
        // 实例化图像编辑器
        ...
        // 重设商品图片宽高
        $editor->resizeExact($goodsImage, 750, 750);
        // 商品图片添加到背景图
        $editor->blend($backdropImage, $goodsImage, 'normal', 1.0, 'top-left', 0, 1);
        // 商品名称处理换行
        $fontSize = 30;
        $goodsName = $this->wrapText($fontSize, 0, $fontPath, $this->goods['goods_name'], 790, 3);
        // 写入商品名称
        //var_dump($fontPath);die;
        $editor->text($backdropImage, $goodsName, 22, 95, 790, new Color('#333333'), $fontPath);//原点在左上角
        // 写入扫描或长按小程序码
        $editor->text($backdropImage, '扫描或长按小程序码', 18, 500, 1110, new Color('#666666'), $fontPath);//原点在左上角
        // 写入商品价格
        $editor->text($backdropImage, '体验价:', 20, 95, 950, new Color('#ff4444'), $fontPath);//原点在左上角
        $priceType = [10 => 'goods_price', 20 => 'sharing_price'];
        $this->goods['sku'][0][$priceType[$this->goodsType]] = '¥'.$this->goods['sku'][0][$priceType[$this->goodsType]];
        $editor->text($backdropImage, $this->goods['sku'][0][$priceType[$this->goodsType]], 22, 195, 950, new Color('#ff4444'));
        // 打开小程序码
        $editor->open($qrcodeImage, $qrcode);
        // 重设小程序码宽高
        $editor->resizeExact($qrcodeImage, 200, 200);
        // 小程序码添加到背景图
        $editor->blend($backdropImage, $qrcodeImage, 'normal', 1.0, 'top-left', 500, 900);
        // 保存图片
