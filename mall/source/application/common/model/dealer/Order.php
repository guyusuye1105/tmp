...
class Order extends BaseModel
{
     ...
     */
    public function supplyMasterOrder(&$list)
    {
        /* @var \think\Collection $list */
        if ($list->isEmpty()) {
            return $list;
        }
        foreach ($list as $item) {
            if ($item['order_type'] == 20) {
                // 拼团订单
                $item['order_master'] = SharingOrderModel::masterDetail($item['order_id']);
            } else {
                // 商城订单
                $item['order_master'] = OrderModel::masterDetail($item['order_id']);
            }
        }
        return $list;
    }
     ...
     * @throws \think\exception\DbException
     */
    public function getOrderMasterAttr($value, $data)
    {
        if ($data['order_type'] == 20) {
            // 拼团订单
            $order = SharingOrderModel::masterDetail($data['order_id']);
        } else {
            // 商城订单
            $order = OrderModel::masterDetail($data['order_id']);
        }
        return $order ? $order->toArray() : $value;
    }
