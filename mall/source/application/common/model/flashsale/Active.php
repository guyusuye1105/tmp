...
{
    protected $name = 'flashsale_active';
    ...
    public static function init()
    {
        ...
        // 监听订单处理事件
        $static = new static;
        Hook::listen('flashsale_active', $static);
    }
    ...
    {
        return static::get($active_id, array_merge(['goods', 'users' => ['user', 'flashsaleOrder']], $with));
    }
    // 根据$active_id获取订单信息
    public function getActive($active_id)
    {
        $res = $this->alias('a')
            ->join('flashsale_order',"flashsale_order.order_id =a.order_id")
...
dump($res);die;
    ...
    public function checkAllowJoin($active,$user_id)
    {
        ...
        $res = db('flashsale_active_users')->where($map)->select();
        if(count($res)>0){
            return array('code'=>2,'msg'=>'当前用户已经帮砍过价了');
        }
        // 判断秒杀是否结束
        if (time() > $active['end_time']) {
            return array('code'=>3,'当前秒杀已结束');
        }
       /*
