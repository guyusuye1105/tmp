...
{
    protected $name = 'flashsale_setting';
    ...
    public static function getAll($wxapp_id = null)
    {
        ...
        if (!$data = Cache::get('flashsale_setting_' . $wxapp_id)) {
            $data = array_column(collection($self::all())->toArray(), null, 'key');
            Cache::tag('cache')->set('flashsale_setting_' . $wxapp_id, $data);
        }
