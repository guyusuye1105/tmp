                ...
                <form id="my-form" class="am-form tpl-form-line-form" method="post">
                    <div class="widget-body">
                        <fieldset>
                            ...
                            <div class="am-form-group">
                                ...
                                <div class="am-u-sm-9 am-u-end">
                                    <input type="text" class="tpl-form-input" name="goods[goods_name]"
                                           value="<?= $model['goods_name'] ?>" maxlength="14" required>
                                </div>
                            </div>
                            <div class="am-form-group">
                            ...
                            <div class="widget-head am-cf">
                                <div class="widget-title am-fl">套餐信息</div>
                                <textarea id="container2" name="goods[package]"><?= $model['package'] ?></textarea>
                            </div>
                            ...
                            <div class="am-form-group">
                                <label class="am-u-sm-3 am-u-lg-2 am-form-label">是否为虚拟商品</label>
                                <div class="am-u-sm-9 am-u-end">

                                    <label class="am-radio-inline">
                                        <input type="radio" name="goods[is_virtual]" value="1" data-am-ucheck
                                            <?= $model['is_virtual'] == 1 ? 'checked' : '' ?> >
                                        是
                                    </label>
                                    <label class="am-radio-inline ">
                                        <input type="radio" name="goods[is_virtual]" value="0" data-am-ucheck
                                            <?= $model['is_virtual'] == 0 ? 'checked' : '' ?> >
                                        否
                                    </label>

                                </div>
                                <small class="am-u-sm-7 am-u-end">虚拟商品自动发货收货 会生成12位串码 通过核销串码完成订单</small>
                            </div>

                            <div class="am-form-group">
                                <label class="am-u-sm-3 am-u-lg-2 am-form-label">是否绑定预约服务</label>
                                <div class="am-u-sm-9 am-u-end">

                                    <label class="am-radio-inline">
                                        <input type="radio" name="goods[is_appoint_time]" value="1" data-am-ucheck
                                            <?= $model['is_appoint_time'] == 1 ? 'checked' : '' ?> >
                                        是
                                    </label>
                                    <label class="am-radio-inline ">
                                        <input type="radio" name="goods[is_appoint_time]" value="0" data-am-ucheck
                                            <?= $model['is_appoint_time'] == 0 ? 'checked' : '' ?> >
                                        否
                                    </label>

                                </div>
                                <small class="am-u-sm-7 am-u-end">绑定预约服务 在购买后需要预约服务时间</small>
                            </div>

                            <div class="appoint-no" <?= $model['is_appoint_time'] == 1 ? 'style="display:none"' : '' ?>>

                            </div>
                            <div class="appoint-yes" <?= $model['is_appoint_time'] == 0 ? 'style="display:none"' : '' ?>>
                                <label class="am-u-sm-3 am-u-lg-2 am-form-label">预约服务</label>
                                <div class="am-u-sm-9 am-u-end">
                               <?php  if (!$subject->isEmpty()): ?>
                                <select name="goods[subject_id]" required
                                        data-am-selected="{searchBox: 1, btnSize: 'sm',  placeholder:'请选择预约项目'}">
                                        <?php foreach ($subject as $first): ?>
                                            <option value="<?= $first['subject_id'] ?>"
                                                <?= $model['subject_id'] == $first['subject_id'] ? 'selected' : '' ?>>
                                                <?= $first['subject_name'] ?></option>
                                        <?php endforeach; ?>
                                </select>
                               <?php  endif; ?>
                                </div>
...
<script>
  $(function () {
    ...
    $('input:radio[name="goods[spec_type]"]').change(function (e) {
      ...
      // 切换绑定预约服务
      $('input:radio[name="goods[is_appoint_time]"]').change(function (e) {
          var $goodsSpecMany2 = $('.appoint-no')
              , $goodsSpecSingle2 = $('.appoint-yes');
          if (e.currentTarget.value == 1) {
              $goodsSpecMany2.hide() && $goodsSpecSingle2.show();
          } else {
              $goodsSpecMany2.show() && $goodsSpecSingle2.hide();
          }
      });
