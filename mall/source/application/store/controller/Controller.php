...
class Controller extends \think\Controller
{
    ...
    private function getLoginData(){
        ...
        if(empty($session['moudle'])){
            //通过cookies 再去刷一次
            $uniacid=Cookie::get('__uniacid');
            if($uniacid){
                $key=$uniacid."_logindata";
                $logindata=cache($key);
                if(!empty($logindata)){
                    $logindata=unserialize($logindata);
                    $module=$logindata['we7_module'];
                    $module="we7_tp_".$module;

                    Session::set('moudle',$module);
                    Session::set($module,$logindata);
                    $session['moudle']=$module;
                    $session[$module]=$logindata;

                    $store=$session[$module];

                    $where['we7_uid']=$logindata['we7_user']['uid'];
                    $where['user_name']=$logindata['we7_user']['user_name'];

                    $user = \app\common\model\StoreUser::where($where)->find();
                    $userdata=$user->getData();
                    unset($userdata['password']);
                    $store['user']=array_merge($userdata,$logindata['we7_user']);
                    $this->store=$store;
                    Session::set($module,$this->store);
                    // 当前用户信息
                }else{
                    throw new BaseException(['msg' => 'Session丢失，请重新进入1']);
                }
            }
        }else{
            ...
            if($store){
                ...
                throw new BaseException(['msg' => 'Session丢失，请重新进入2']);
            }
        }
    ...
    private function checkPrivilege()
    {
        ...
        if (!Auth::getInstance()->checkPrivilege($this->routeUri)) {
            ...
            $this->error("很抱歉，没有访问权限,3秒后返回首页",$we7_url);
