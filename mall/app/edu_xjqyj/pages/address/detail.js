...
Page({
  ...
  bindDateChange(e){
    this.setData({
      'detail.extra.kid_age':e.detail.value
    })
  },
  /**
   * 生命周期函数--监听页面加载
  ...
  getAddressDetail: function(address_id) {
    ...
    App._get('address/detail', {
      ...
      if (result.data.detail.extra){
        result.data.detail.extra = JSON.parse(result.data.detail.extra)
      }
  ...
  saveData: function(e) {
    ...
    values.kid_age = _this.data.detail.extra.kid_age
    // 提交到后端
  ...
  validation: function(values) {
    ...
    if (values.kid_name === '') {
      this.data.error = '孩子姓名不能为空';
      return false;
    }
    if (values.kid_age === '') {
      this.data.error = '孩子年龄不能为空';
      return false;
    }
    return true;
  },
